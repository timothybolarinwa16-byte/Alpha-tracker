      ...
      Array.isArray(
        ...
        ? data.symbols
            .filter(
              item =>
                item.status === "TRADING" &&
                item.contractType !== "PERPETUAL_DELIVERING"
            )
            .map(
              item =>
...
async function fetchHistoricalTrades(
  ...
  if (
    ...
    throw new Error(
      "Futures exchange unavailable"
    );

  }

  return state.exchange.getHistoricalTrades(
    symbol,
    startTime,
    endTime,
    1000
...
async function initialize() {
  ...
  /*
   * The Futures module is defined
   * directly inside this app.js.
   *
   * No separate BinanceExchange
   * script is required.
   */
